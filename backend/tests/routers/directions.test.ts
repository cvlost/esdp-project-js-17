...
describe('directionRouter', () => {
  ...
  describe('PUT /directions/:id', () => {
    test('неавторизованный пользователь пытается редактировать 1 запись, должен возвращать statusCode 401 и сообщение об ошибке', async () => {
      const res = await request
        .put(`/directions/${directionIdNotRelatedToLocation}`)
    ...
    describe('авторизованный пользователь пытается редактировать 1 запись', () => {
      ...
      test('указав корректный id и валидные данные, должен возвращать statusCode 200 и объект измененной записи', async () => {
        const updateDto = { name: 'updated direction 1' };
        const res = await request
          .put(`/directions/${directionIdNotRelatedToLocation}`)
          .set({ Authorization: adminToken })
          .send(updateDto);
        ...
        const updatedDirection = await Direction.findById(directionIdNotRelatedToLocation);
        expect(updatedDirection).toBeTruthy();

        if (updatedDirection) {
          expect(updatedDirection.name).toBe(updateDto.name);
        }
      });
    });
  });
